...
{
    #define UPDATE_PROPERTY_FLOAT(name) (device_twin_update_float(&environment->latest.pollution.name, &environment->previous.pollution.name, &dt_ ## name))
    #define UPDATE_PROPERTY_INT(name) (device_twin_update_int(&environment->latest.weather.name, &environment->previous.weather.name, &dt_ ## name))
...
void publish_telemetry(ENVIRONMENT_TELEMETRY *environment)
{
    ...
    size_t msg_len = snprintf(msgBuffer, sizeof(msgBuffer), publish_template,
