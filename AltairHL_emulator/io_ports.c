        ...
        case 31:
            if (!publish_pending)
            {
                ...
                if (data == 0)
                {
                    ...
                    dx_timerOneShotSet(&tmr_deferred_port_out_json, &(struct timespec){0, 500 * ONE_MS});
                }
            }
            break;
        case 32:
            if (!publish_weather_pending)
            {
                ...
                // Throttle IoT messages to 1 second
                dx_timerOneShotSet(&tmr_deferred_port_out_weather, &(struct timespec){0, 500 * ONE_MS});
            }
            break;
